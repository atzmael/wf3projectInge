<?php
	...
	$query = $db -> query("SELECT article.id_article, article.title, article.description, article.release_date, article.vote, user.pseudo FROM article JOIN user ON article._id_util = user.id_util WHERE article._id_lang = 3");
	$query -> execute();
